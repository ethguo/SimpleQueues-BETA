	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length > 2 && args[0].equalsIgnoreCase("create")) {
			...
			if (args.length == 3 && sender instanceof Player) {
				...
				// ./qconfig create <name> <size> [tpX tpY tpZ [tpWorld]]
				World tpW;
				if (args.length == 6) {
					if (sender instanceof Player) {
						tpW = ((Player) sender).getWorld();
					}
					else {
						// Get the world
						tpW = plugin.getServer().getWorld("world");
						if (tpW == null) {
							sender.sendMessage("Could not find a world. Please include the world parameter. (This could be the name of the current world folder)");
							return false;
						}
					}
				}
					...
					// Get the world
					tpW = plugin.getServer().getWorld(args[6]);
					if (tpW == null) {
						sender.sendMessage(args[6] + " is not a valid world.");
						return true;
					}
				}
				else {
				...
				try {
					queueTpLocation = Helpers.parseLocation(plugin, args[3], args[4], args[5], tpW);
				}
