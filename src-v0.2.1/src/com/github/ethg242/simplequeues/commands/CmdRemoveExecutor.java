	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (args.length > 0) {
			...
			if (args.length == 1 && sender instanceof Player && sender.hasPermission("SimpleQueues.remove.self")) {
				...
				if (!success) {
					sender.sendMessage(MessageColor.PROBLEM + "You are not in the queue " + MessageColor.HIGHLIGHT + queue.name);
				}
				sender.sendMessage(MessageColor.NORMAL + "You have been removed from the queue " + MessageColor.HIGHLIGHT + queue.name);
				plugin.getServer().getLogger().fine(sender.getName() + " has been removed from the queue " + queue.name);
				return true;
			}
