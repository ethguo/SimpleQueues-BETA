	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length >= 3 && args[0].equalsIgnoreCase("create")) {
			...
			// ./qconfig create <name> <size> and sender is a player
				...
				sender.sendMessage("You must either provide coordinates or be a player.");
			}
			
			// If queues is full, notify sender
			
			boolean success = plugin.createQueue(args[1], queueSize, queueTpLocation);
			
			if (!success) {
				sender.sendMessage("There are already " + plugin.queues.length + " queues on the server.");
				return true;
			}
			
			sender.sendMessage("Successfully created queue " + args[1] + " with size " + queueSize + " and tplocation " + queueTpLocation.toString());
		...
		// Deletes a queue
			...
			if (success) {
				sender.sendMessage("Successfully deleted queue " + queue.name);
				plugin.getLogger().info("Deleted queue " + args[1]);
			} else {
				sender.sendMessage("Was not able to delete queue " + queue.name);
			}
			return true;
		}
		return false;
